...
export function AssetsTable({ activos }: Props) {
  ...
  return (
    <table className="w-full border-collapse text-sm">
      ...
      <tbody>
        {activos.map((activo) => (
          <tr key={activo.id} className="border-b border-slate-100">
            <td className="py-2 pr-4 font-medium text-slate-800">{activo.nombre}</td>
            <td className="py-2 pr-4 text-slate-600">{activo.categoria.nombre}</td>
            <td className="py-2 pr-4 text-slate-600">{activo.criticidad}</td>
            <td className="py-2 pr-4 text-slate-600">{activo.usuarioResponsable.nombre}</td>
            ...
            <td className="py-2 pr-4 space-x-3">
              <Link to={`/activos/${activo.id}/editar`} className="text-slate-700 underline">
                Editar
              </Link>
